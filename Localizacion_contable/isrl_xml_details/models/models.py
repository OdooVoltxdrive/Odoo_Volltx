...
class XmlLines(models.Model):
    ...
    def generar_xml(self):
        ...
        for item in self.line_id:
            ...
            if item.numero_factura:
                # 1. Quitamos guiones y espacios en blanco por seguridad
                nro_fact = item.numero_factura.replace('-', '').strip()
                
                # 2. Rellenamos con ceros a la izquierda hasta completar 10 caracteres
                # Si ya tiene 10 o más, .zfill no le hace nada.
                # Luego usamos [-10:] para asegurar que si tiene más de 10, solo tome los últimos.
                nro_fact = nro_fact.zfill(10)[-10:]
            else:
                # Si no hay factura, el estándar suele pedir los 10 ceros
                nro_fact = '0'.zfill(10)
    ...
    def create_xml(self):
        ...
        for item in retencion:
            for line in item.lines_id:
                ...
                'monto_operacion' : str(round(line.base,2)),
